...
| 縮小した軸 | 本追試 | 原論文 [1] | 縮小の理由 |
|---|---|---|---|
| 試行数 N_ALL | 2,500 / side（= 5,000 fit） | 5,000 / side（= 10,000 fit） | **計算時間**（実測の外挿で最終決定）[計画] |
| シード | 3 本程度 | 多数 | 計算時間 |
...
**所要時間（暫定見積もり）**: N_ALL=2,500/side（= 5,000 fit）で約 **40–65 分**と見積もる[計画]（[scripts/run_all.sh](scripts/run_all.sh) のコメント）。これを線形外挿すると、フル再現 C（10,000 fit・単一 (ε,δ)）は本実行環境で概ね **1.5–2.5 時間規模**になる。ただし**現時点で実測値は未取得**である（outputs/ は空、E0 スモーク未実行）。正確な per-fit 時間と外挿は E0 スモーク後に確定する[計画]。
...
<!-- TODO: E0 スモーク（例 N_ALL=50）後に per-fit 実測・縮小版総時間・確定 N_ALL を埋める。 -->
