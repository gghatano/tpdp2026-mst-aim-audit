...
## 3. 強い敵対者モデル（strong adversary）

DP の保証は**強い敵対者**（機構 M と 2 つの隣接データセット x, x' を知り、ただ「どちらが使われたか」だけを知らない敵）に対して成り立つことを要求する[A19]。監査もこの強い敵対者を模す。本手法の敵対者は hybrid black/white-box である[1]:

- **ブラックボックス特徴**: 合成データに対し離散ドメインの**全クエリ**を評価したカウント（Query-based MIA [A17] に従う）[1]。
...
> 🔎 **直感**: 強い敵対者を仮定するほど経験的下界は上がり（tight に近づき）、現実的な弱い敵を仮定するほど下界は緩む。監査は「保証が壊れていないか」を厳しく問うため、あえて強い敵対者を使う。
...
- 区別ゲーム＋強い敵対者＋worst-case カナリアが tight な監査の三要素[1][A15][A16]。
...
> 本ページの出典タグ（[A1] など）はこの表の各項目へリンクする。末尾の「REPORT の [Rn]」は本体レポートの References の対応番号。
...
- **[A1]** Annamalai, Ganev, De Cristofaro. *Experimenting with Tight Auditing of DP Synthetic Data Generation*. USENIX Security, 2024.（採用した監査枠組み・従来の μ_emp=0。REPORT の [R1]）
- **[A12]** Annamalai et al. *SoK: The Hitchhiker Guide to Efficient, End-to-End, and Tight DP Auditing*. IEEE SaTML, 2026.（DP 監査総説。REPORT の [R12]）
- **[A15]** Nasr et al. *Adversary Instantiation*. IEEE S&P, 2021.（worst-case 構成。REPORT の [R15]）
- **[A16]** Nasr et al. *Tight Auditing of DP Machine Learning*. USENIX Security, 2023.（ベイズ下界の被覆に関する注意。REPORT の [R16]）
- **[A17]** Houssiau et al. *TAPAS*. NeurIPS Workshop, 2022.（Query-based MIA。REPORT の [R17]）
- **[A18]** Zanella-Béguelin et al. *Bayesian Estimation of Differential Privacy*. ICML, 2023.（joint Bayesian 下界。REPORT の [R18]）
- **[A19]** Dong, Roth, Su. *Gaussian Differential Privacy*. JRSSB, 2022.（強い敵対者・仮説検定の視点。REPORT の [R19]）
