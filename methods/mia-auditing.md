...
> 技術解説ページ（モードA）。[REPORT.md](REPORT.html) §2.1・§2.3・§2.4 から参照される補助資料。図ラベルは英語、本文は日本語。事実は出典タグ `[n]`（末尾の対応表）、筆者の補足は 🔎。
...
この (FPR, FNR) を理論フロンティアと比較し、μ-GDP の経験的 μ に変換する[1]（→ [methods/gdp-and-tradeoff.md](gdp-and-tradeoff.html)）。
